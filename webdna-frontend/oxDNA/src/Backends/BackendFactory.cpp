...
#include "../CUDA/Backends/MD_SP_CUDAMPIBackend.h"
#endif
...
#ifdef HAVE_MPI
			if(!strcmp(backend_prec, "double")) new_backend = new MD_SP_MPIBackend<double, d_quadtype>();
			else if(!strcmp(backend_prec, "float")) new_backend = new MD_SP_MPIBackend<float, f_quadtype>();
			else throw oxDNAException("Backend precision '%s' is not supported", backend_prec);
			int procsize;
...
#ifndef NOCUDA            
            if (!strcmp(backend_prec, "double")) new_backend = new MD_SP_CUDAMPIBackend<double, LR_double4>();
            else if (!strcmp(backend_prec, "float")) new_backend = new MD_SP_CUDAMPIBackend<float, float4>();
            else throw oxDNAException("Backend precision '%s' is not supported", backend_prec);
#endif
#endif
