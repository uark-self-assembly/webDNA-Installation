...
void Cells<number>::global_update() {
	...
	while(_N_cells_side > 500) _N_cells_side--;
		...
		for(int k = -1; k < 2; k++) {
			...
			for(int l = -1; l < 2; l++) {
				...
				while(q != P_VIRTUAL) {
                    ...
                    if (this->_bonded)
                    {
                        if(p->pos.sqr_min_image_distance(q->pos, this->_box) < _sqr_rcut)
                        {
                            res.push_back(q);
                        }
                    }
                    ...
                    {
                        if((p->index < q->index or (this->_is_MC and p != q)) and
                           !p->is_bonded(q) and
                           p->pos.sqr_min_image_distance(q->pos, this->_box) < _sqr_rcut)
                        {
                            res.push_back(q);
                        }
                    }
