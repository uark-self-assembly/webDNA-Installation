...
void MDBackend<number>::_generate_vel() {
	...
	for(int i = 0; i < this->_N; i++) {
		...
		p->vel.x = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());
		p->vel.y = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());
		p->vel.z = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());

		p->L.x = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());
		p->L.y = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());
		p->L.z = Utils::gaussian<number>() * (rescale_factor * p->get_scale_factor());
