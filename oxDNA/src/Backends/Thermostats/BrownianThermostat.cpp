...
void BrownianThermostat<number>::apply (BaseParticle<number> **particles, llint curr_step) {
	...
	for(int i = 0; i < this->_N_part; i++) {
		...
		if(drand48() < _pt) {
			p->vel = LR_vector<number>(Utils::gaussian<number>(), Utils::gaussian<number>(), Utils::gaussian<number>()) * (_rescale_factor * p->get_scale_factor()); //TLF change added '* p->get_scale_factor()
		}
		if(drand48() < _pr) {
			p->L = LR_vector<number>(Utils::gaussian<number>(), Utils::gaussian<number>(), Utils::gaussian<number>()) * (_rescale_factor * p->get_scale_factor()); //TLF change added '* p->get_scale_factor()
		}
	}
}
