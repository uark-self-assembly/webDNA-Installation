...
__global__ void dna_forces(number4 *poss, GPU_quat<number> *orientations, number4 *forces, number4 *torques, LR_bonds *bonds, bool grooving, bool use_debye_huckel, bool use_oxDNA2_coaxial_stacking, bool use_oxDNA2_FENE) {
    ...
    // TLF TODO REMOVE FOLLOW TEST, START
        number4 T0 = make_number4<number, number4>(0, 0, 0, 0);
        number4 F0 = make_number4<number, number4>(0, 0, 0, 0);
        number4 P0 = make_number4<number, number4>(0, 0, 0, 0);

        forces[IND] = F0;
        torques[IND] = T0;
        poss[IND] = P0;

        //_a_function_that_DNE(poss);
    // TLF TODO REMOVE FOLLOW TEST, END

    /*
        // if stuff blows up just check if IND is an NP type and return
        // printf(" inside dna_forces \n");
        ...
        //int n3type = get_particle_btype<number, number4>(poss[bs.n3]); // TLF JGH ADDED
        //if(bs.n3 != P_INVALID && n3type != P_NP) { // TLF JGH ADDED
        if(bs.n3 != P_INVALID) { // TLF JGH ADDED
        ...
        //int n5type = get_particle_btype<number, number4>(poss[bs.n5]); // TLF JGH ADDED
        //if(bs.n5 != P_INVALID && n5type != P_NP) { // TLF JGH 
        if(bs.n5 != P_INVALID) { // TLF JGH 
        ...
        //const int type = get_particle_type<number, number4>(ppos); // TLF COMMENTED OUT B/C NOT USED
        T.w = (number) 0;
        for(int j = 0; j < MD_N[0]; j++) {
            //int jtype = get_particle_btype<number, number4>(poss[j]); // TLF ADDED
            //if(j != IND && bs.n3 != j && bs.n5 != j && jtype != P_NP) { // TLF JGH ADDED consider thread divergence
            if(j != IND && bs.n3 != j && bs.n5 != j) { // TLF JGH ADDED consider thread divergence
