...
void MD_CUDABackend<number, number4>::_host_particles_to_gpu() {
	for(int i = 0; i < this->_N; i++) {
        ...
        this->_h_massinvs[i] = p->massinv; // TLF JGH added
        ...
        // tlf remove
        //cout << myindex << ") type: " << p->type;
        //cout << ", p btype: " << p->btype << endl;
        //cout << ", btype: " << mybtype << endl;
...
void MD_CUDABackend<number, number4>::_gpu_to_host_particles() {
	...
	for(int i = 0; i < this->_N; i++) {
        ...
        // TLF => why do we transfer 'p->btype' back to the hoes, or rather recalculate it.
		// get index and type for the fourth component of the position
		p->btype = (GpuUtils::float_as_int(this->_h_poss[i].w)) >> 22;
        //cout << "h_poss[i].w ("<< i << ") " << this->_h_poss[i].w << endl;
        //cout << i << ") " << GpuUtils::float_as_int(this->_h_poss[i].w) << "\t\t" << (GpuUtils::float_as_int(this->_h_poss[i].w) >> 22) << "\t\t" << this->_h_poss[i].w << endl;
