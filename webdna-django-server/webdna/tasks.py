...
def generate_initial_configuration(project_id: str, log_file_path: str) -> str:
    ...
    elif generation.method == 'uploaded':
        if not server.project_file_exists(project_id, ProjectFile.UPLOADED_DAT) \
        or not server.project_file_exists(project_id, ProjectFile.UPLOADED_TOP):
            return messages.MISSING_PROJECT_FILES

        if not oxdna.uploaded(project_id, generation, log_file_path):
            return messages.INTERNAL_ERROR
        else:
            return messages.GENERATED_FILES
