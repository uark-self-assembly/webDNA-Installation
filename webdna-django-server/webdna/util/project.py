...
class Generation:
    def __init__(self, method: str = None, arguments: List = None, orig: 'Generation' = None, dictionary: Dict = None):
        if orig is not None:
            ...
            elif method == 'uploaded':
                self.files = [ProjectFile.UPLOADED_DAT.value] + [ProjectFile.UPLOADED_TOP.value]
                self.arguments = arguments + self.files
    ...
    if generation_method == 'generate-sa' or generation_method == 'generate-folded':
        if regenerate:
            ...
            if os.path.isfile(generated_dat) and os.path.isfile(generated_top):
                ...
                return False
    elif generation_method == 'uploaded':
        if regenerate: 
            uploaded_dat = server.get_project_file(project_id, ProjectFile.UPLOADED_DAT)
            uploaded_top = server.get_project_file(project_id, ProjectFile.UPLOADED_TOP)
            if os.path.isfile(uploaded_dat) and os.path.isfile(uploaded_top):
                return True
            else: 
                return False
        else:
            generated_top = server.get_project_file(project_id, ProjectFile.UPLOADED_TOP)
            generated_dat = server.get_project_file(project_id, ProjectFile.UPLOADED_DAT)
            if os.path.isfile(generated_dat) and os.path.isfile(generated_top):
                return True
            else:
                return False
